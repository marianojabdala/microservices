...
class Config(object):
    ...
    DB_USER_PASS = DB_USER + ":" + os.getenv("DB_PASSWORD") if DB_USER  else ""
    DB_HOST = f"{DB_USER_PASS}{'@'}{os.getenv('DB_HOST') if DB_USER_PASS else None}"
    SQLALCHEMY_DATABASE_URI = f"{os.getenv('DB_PREFIX')}{DB_HOST if DB_HOST else PATH}/" \
                              f"{DATABASE}{os.getenv('DB_SUFIX')}"
...
class DevelopmentConfig(Config):
    ...
    SQLALCHEMY_DATABASE_URI = f"{os.getenv('DB_PREFIX')}{Config.DB_HOST if Config.DB_HOST else Config.PATH }/" \
                              f"{os.getenv('DATABASE')}{os.getenv('DB_SUFIX')}"
